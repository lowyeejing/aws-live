...
def updateEmpOutput():
    ...
    update_sql = "UPDATE INTO employee SET first_name = %(first_name)s , last_name = %(last_name)s AN,D pri_skill = %(pri_skill)s , location = %(location)s , emp_image_file = %(emp_image_file)s) WHERE emp_id = %(emp_id)s"
    cursor = db_conn.cursor()
